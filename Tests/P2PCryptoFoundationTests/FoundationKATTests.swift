// FoundationKATTests.swift
// KAT/RFC vectors for the FoundationCryptoProvider (the cross-provider reference).
// These confirm the swift-crypto/CryptoKit adapter produces the same standard
// outputs the Embedded provider is pinned against.
import Testing
import P2PCoreCrypto
@testable import P2PCryptoFoundation

@Suite("Foundation AEAD KAT")
struct FoundationAEADTests {
    @Test func chaCha20Poly1305RFC8439() throws {
        ...
        let aead = try FoundationAEAD(algorithm: .chacha20poly1305, key: key.span)
    ...
    @Test func aes128GCMNIST() throws {
        ...
        let aead = try FoundationAEAD(algorithm: .aes128gcm, key: key.span)
...
@Suite("Foundation Hash/HKDF/HMAC KAT")
struct FoundationDigestTests {
    @Test func sha256ABC() {
        let input = Array("abc".utf8)
        #expect(FoundationSHA256.hash(input.span) == Hex.decode("ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad"))
    }

    @Test func sha384ABC() {
        let input = Array("abc".utf8)
        #expect(FoundationSHA384.hash(input.span) == Hex.decode("cb00753f45a35e8bb5a03d699ac65007272c32ab0eded1631a8b605a43ff5bed8086072ba1e7cc2358baeca134c825a7"))
    }
    ...
    @Test func hmacSHA256RFC4231TC2() {
        ...
        #expect(FoundationHMACSHA256.authenticationCode(for: data.span, key: key.span)
    ...
    @Test func hmacSHA1RFC2202TC2() {
        let key = Array("Jefe".utf8)
        let data = Array("what do ya want for nothing?".utf8)
        #expect(FoundationHMACSHA1.authenticationCode(for: data.span, key: key.span)
                == Hex.decode("effcdf6ae5eb2fa2d27416d5f184df9c259a7c79"))
    }
}

@Suite("Foundation Curve/Signature KAT")
struct FoundationCurveTests {
    @Test func x25519RFC7748() throws {
        ...
        let priv = try FoundationX25519.privateKey(rawRepresentation: scalar.span)
        let peer = try FoundationX25519.publicKey(rawRepresentation: uCoordinate.span)
        let shared = try FoundationX25519.sharedSecret(privateKey: priv, peerPublicKey: peer)
    ...
    @Test func p256ECDHRoundtrip() throws {
        let a = try FoundationP256Agreement.generatePrivateKey()
        let b = try FoundationP256Agreement.generatePrivateKey()
        let aPub = FoundationP256Agreement.publicKey(for: a)
        let bPub = FoundationP256Agreement.publicKey(for: b)
        let ab = try FoundationP256Agreement.sharedSecret(privateKey: a, peerPublicKey: bPub)
        let ba = try FoundationP256Agreement.sharedSecret(privateKey: b, peerPublicKey: aPub)
        #expect(ab == ba)
    }

    @Test func ed25519SignVerify() throws {
        let signingKey = try FoundationEd25519.generateSigningKey()
        let verifyingKey = FoundationEd25519.verifyingKey(for: signingKey)
        let message = Array("hello".utf8)
        let sig = try FoundationEd25519.sign(message.span, with: signingKey)
        let valid = FoundationEd25519.isValid(signature: sig.span, for: message.span, with: verifyingKey)
        #expect(valid)
    }

    @Test func p256ECDSASignVerify() throws {
        let signingKey = try FoundationP256Signature.generateSigningKey()
        let verifyingKey = FoundationP256Signature.verifyingKey(for: signingKey)
        let message = Array("the message to sign".utf8)
        let sig = try FoundationP256Signature.sign(message.span, with: signingKey)
        let valid = FoundationP256Signature.isValid(signature: sig.span, for: message.span, with: verifyingKey)
        #expect(valid)
    }
}

@Suite("Foundation HeaderProtection KAT")
struct FoundationHeaderProtectionTests {
    @Test func aesHeaderProtectionRFC9001() throws {
        ...
        let mask = try FoundationHeaderProtection.aesECBBlockMask(key: hpKey.span, sample: sample.span)
    ...
    @Test func chaCha20HeaderProtectionRFC9001() throws {
        ...
        let mask = try FoundationHeaderProtection.chaCha20BlockMask(key: hpKey.span, sample: sample.span)
