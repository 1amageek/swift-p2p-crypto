...
// DUAL-MODE VENDORED CHECKOUT for swift-p2p-crypto (M4).
//
// This is a copy of the empirically-validated spike checkout (BoringSSL commit
// 0226f30467f540a3f62ef48d453f93927da199b6). Only this manifest differs from the
// spike; no BoringSSL/Swift source is modified.
//
// It serves BOTH M4 providers from one package (avoiding the two-swift-crypto
// target-name collision):
//   - P2PCryptoEmbedded consumes the C BoringSSL targets via the
//     `CBoringSSLForProbe` product. C compilation is clang-driven and unaffected
//     by the Embedded Swift flag, which is applied by the *consuming* target.
//   - P2PCryptoFoundation consumes the high-level `Crypto` / `_CryptoExtras`
//     products, which must build host-clean (CryptoKit on Apple) — i.e. NOT with
//     the Embedded swiftSettings.
//
// Hence `development = false`: the high-level Swift targets use the normal
// (non-Embedded) settings. The C BoringSSL targets remain available on macOS via
// the product declaration below.
...
if development || isFreeBSD {
    swiftSettings = [
        ...
        .define("CRYPTOKIT_NO_IMPORT_FOUNDATION"),
...
// resource.
// EMBEDDED SPIKE: in development/Embedded mode we must NOT declare the privacy-manifest
// resource. SPM auto-generates a `Bundle.module` accessor that `import Foundation`, which
// cannot compile under Embedded. Drop the resource entirely on this path.
#if canImport(Darwin)
...
let package = Package(
    ...
    products: [
        ...
        // PROBE-ONLY: expose the C BoringSSL targets so the crypto-bind-probe
        // package can validate Embedded bindings without importing high-level Crypto.
        .library(name: "CBoringSSLForProbe", targets: ["CCryptoBoringSSL", "CCryptoBoringSSLShims"]),
