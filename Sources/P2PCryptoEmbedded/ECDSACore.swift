...
enum ECDSACore {
    ...
    static func sign(
        ...
        let ok = sig.withUnsafeMutableBufferPointer { sp in
            digest.withUnsafeBufferPointer { dp in
                CP2PBoringSSL_ECDSA_sign_p1363(
    ...
    static func verify(
        ...
        guard let group = CP2PBoringSSL_EC_GROUP_new_by_curve_name(curve.nid) else { return false }
        defer { CP2PBoringSSL_EC_GROUP_free(group) }
        guard let key = CP2PBoringSSL_EC_KEY_new_by_curve_name(curve.nid) else { return false }
        let managed = ManagedECKey(key: key)
        guard let point = CP2PBoringSSL_EC_POINT_new(group) else { return false }
        ...
        guard CP2PBoringSSL_EC_KEY_set_public_key(managed.key, point) == 1 else { return false }

        let ok = digest.withUnsafeBufferPointer { dp in
            signature.withUnsafeBufferPointer { sp in
                CP2PBoringSSL_ECDSA_verify_p1363(
