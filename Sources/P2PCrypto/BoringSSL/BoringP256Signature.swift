// BoringP256Signature.swift
// ECDSA P-256 sign/verify (crypto-impl.md §2.8). Signs SHA-256(message); raw
// r||s signatures (64 B). Signing key = raw scalar (32 B, zeroized); verifying
// key = X9.62 uncompressed point (65 B).
import P2PCoreCrypto

/// ECDSA over P-256. Conforms `P2PCoreCrypto.SignatureScheme`.
public enum BoringP256Signature: SignatureScheme {
    static let curve = ECCurve.p256
    static let signatureLength = 64

    /// A P-256 signing scalar, zeroized on drop (§5.2).
    public struct SigningKey: Sendable {
        let secret: SecretBytes
        init(_ bytes: [UInt8]) { self.secret = SecretBytes(bytes) }
    }

    /// A P-256 verifying key as an X9.62 uncompressed point (65 B).
    ...
    public static func verifyingKey(for signingKey: SigningKey) -> VerifyingKey {
        do {
            ...
            fatalError("BoringP256Signature: public-key derivation failed for a validated scalar")
        }
    }
    ...
    public static func sign(_ message: Span<UInt8>, with signingKey: SigningKey) throws(CryptoError) -> [UInt8] {
        let digest = BoringSHA256.hash(message)
